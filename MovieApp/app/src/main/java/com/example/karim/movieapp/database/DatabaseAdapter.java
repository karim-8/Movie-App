...
public class DatabaseAdapter  {
    ...
    public boolean addMovie(Movie movie)
    {
       ...
       contentValues.put("id",movie.id);
        ...
        contentValues.put(SQLiteHelper.RELEASEDATE,movie.releaseDate);
        contentValues.put(SQLiteHelper.RATE,movie.rate);
    ...
    public List<Movie> getAllMovies ()
    {
        ...
        if(cursor.moveToFirst()){
            do {
                movies.add(new Movie(cursor.getInt(0),cursor.getString(1), cursor.getString(2),cursor.getString(5),
    ...
    public boolean isMovieBookmarked(String title){
        ...
        Cursor cursor = sqLiteDatabase.rawQuery("select * from `moviestable` where `_title`='"+title+"'",null);
        if(cursor.moveToFirst()){
            isBookmarked = true;
        }
        cursor.close();
...
public class SQLiteHelper extends SQLiteOpenHelper {
    ...
    private static final String CREATE_TABLE= "CREATE TABLE "+ TABLE_NAME
            ...
            + " (`id` integer " + TITLE + " VARCHAR(255), " + IMAGE
            + " VARCHAR(255), " + RATE
            + " VARCHAR(255), "+ RELEASEDATE
            + " VARCHAR(255), " + OVERVIEW
            + " VARCHAR(255)); ";
